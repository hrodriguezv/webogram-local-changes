...
angular.module('myApp.controllers', [])
  ...
  .controller('AppImHistoryController', function ($scope, $location, $timeout, $rootScope, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ApiUpdatesManager, PeersSelectService, IdleManager, StatusManager, ErrorService) {
    ...
    function applyDialogSelect (newPeer) {
      ...
      if (!peerID){
    	  safeReplaceObject($scope.state, {notSelected: false});
    ...
    function updateHistoryPeer(preload) {
      ...
      if (peerID == 1) { peerData.deleted = false;}
  ...
  .controller('AppImSendController', function ($scope, $timeout, MtpApiManager, AppConfigManager, AppPeersManager, AppMessagesManager, ApiUpdatesManager, MtpApiFileManager, DhatacasChatMessageService) {
	var tempID = -1;
    ...
    $scope.sendChatMessage = sendChatMessage;
      ...
      $timeout(function () {
        ...
        do {
          (function (peerID, curText, curTimeout) {
            setTimeout(function () {
            	var msgID = tempID--;
            	/*ENVIO DE MENSAJE PARA GAE*/
                var message = {
            	        _: 'message',
            	        id: msgID,
            	        from_id: $scope.$id,
            	        to_id: $scope.curDialog.peerID,
            	        out: true,
            	        unread: true,
            	        date: tsNow() / 1000,
            	        message: curText,
    ...
    function onTyping () {
      if ($scope.curDialog.inputPeer.user_id == '')
    	  return;
    ...
    function createToken() {
		  DhatacasChatMessageService.getToken($scope.$id)
          .then(function(data) {
        	  $scope.token.text = data.data;
        	  channel = new goog.appengine.Channel($scope.token.text);
              socket = channel.open();
             ...
             // error handling here
        	 //alert('Recibiendo Error = ' + error);
          });
	  }
	  
	  function sendChatMessage(text) {
		  if (!text){
	    	  DhatacasChatMessageService.sendChatMessage($scope.$id,$scope.draftMessage.text);  			  
		  }else{
	    	  DhatacasChatMessageService.sendChatMessage($scope.$id,text);  
		  }
