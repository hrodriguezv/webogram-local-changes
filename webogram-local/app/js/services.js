...
//    text = "<html><head><title>Make and Play Udacity 2048</title></head><body>Hola Mundo!</body></html>";
...
.service('DhatacasChatMessageService', function($http) {
	delete $http.defaults.headers.common['X-Requested-With'];
	this.getToken = function(channelKey) {
		// $http() returns a $promise that we can add handlers with
		// .then()
		return $http({
			method : 'GET',
//			url : 'http://disco-glass-634.appspot.com/token?c='+channelKey
			url : 'http://localhost:8080/token?c='+channelKey
		});
	}
	
	this.sendChatMessage = function(channelKey, message){
		return $http({
			method : 'POST',
//			url : 'http://disco-glass-634.appspot.com/chat?channelKey='+channelKey+'&message='+message
			url : 'http://localhost:8080/chat?channelKey='+channelKey+'&message='+message
		});
	}
});
